...
handle_request(ReqKey,
	...
	allocate_ips(APNOpts, ActiveSessionOpts0, EUA, DAF, PCO, ContextVRF),
...
pco_requested_opts(#protocol_configuration_options{config = Requested}, SOpts) ->
    ergw_gsn_lib:pco_requested_opts(Requested, SOpts);
pco_requested_opts(_, _) ->
    [].

allocate_ips(APNOpts, SOpts, EUA, DAF, PCO, Context) ->
    PCOReqOpts = pco_requested_opts(PCO, SOpts),
    ergw_gsn_lib:allocate_ips(pdp_alloc(EUA), APNOpts, SOpts, DAF, PCOReqOpts, Context).
